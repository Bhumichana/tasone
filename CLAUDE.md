...
**CRITICAL**: Tailwind CSS v4 has different configuration requirements:

1. **globals.css syntax**:
   ```css
   @import "tailwindcss";
...
3. **Required dependencies**:
   - Install: `npm install --save-dev autoprefixer @tailwindcss/postcss`
   - Tailwind v4 requires `@tailwindcss/postcss` plugin, not the direct `tailwindcss` plugin

4. **Troubleshooting steps when styling doesn't work**:
   - Delete `.next` directory: `powershell "Remove-Item .next -Recurse -Force"`
   - Kill all processes on port 3000: `taskkill //F //PID [PID]`
   - Restart development server: `npm run dev`
   - Check CSS compilation by fetching: `/_next/static/css/[filename].css`
   - Remove problematic `@layer base` directives that use undefined utilities

5. **Common error messages**:
   - "It looks like you're trying to use `tailwindcss` directly as a PostCSS plugin" → Use `@tailwindcss/postcss`
   - "Cannot apply unknown utility class `border-border`" → Remove incompatible `@layer` directives

6. **CSS Specificity Issues**:
   - **Problem**: Custom `!important` rules can override responsive utilities
   - **Symptom**: Elements with `hidden lg:flex` classes remain hidden on desktop
   - **Solution**: Add specific responsive overrides with `!important`
   ```css
   ...
   ```
   - **Debug tip**: Check DOM for conflicting classes like `hidden lg:flex` that should show on desktop
...
4. **Kill the process** using the appropriate command based on detected OS:
   ...
   - **Windows (PowerShell)**: Use `powershell ".\kill-port.ps1"` for port 3000 or `powershell ".\kill-port.ps1 -Port [number]"` for other ports
   - **Windows (Alternative)**: `Get-Process -Id (Get-NetTCPConnection -LocalPort 3000).OwningProcess | Stop-Process -Force`
